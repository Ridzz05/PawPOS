...
func TestLoadUsesSafeDefaults(t *testing.T) {
	for _, key := range []string{"API_ADDRESS", "DATABASE_URL", "APP_ENV", "LOG_LEVEL", "GROQ_API_KEY", "STT_MODEL", "LLM_MODEL", "AI_AUDIO_MAX_SECONDS", "AI_ENABLED", "ELEVENLABS_API_KEY", "ELEVENLABS_VOICE_ID", "ELEVENLABS_MODEL"} {
		t.Setenv(key, "")
	}
	got := Load()
	...
	if got.GroqAPIKey != "" || got.STTModel != "whisper-large-v3-turbo" || got.LLMModel != "openai/gpt-oss-120b" || got.AIAudioMaxSeconds != 60 || got.AIEnabled || got.ElevenLabsAPIKey != "" || got.ElevenLabsVoiceID != "Xb7hH8MSUJpSbSDYk0k2" || got.ElevenLabsModel != "eleven_multilingual_v2" {
